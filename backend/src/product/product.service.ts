...
import './components/laptop.entity';
...
import { Laptop } from "./components/laptop.entity";
...
export class ProductService {
  ...
  async getProductById(id: string): Promise<any | null> {
    ...
    switch (catId) {
      case CATEGORY_MAP.cpu:
        detail = await this.cpuRepository.findOne({ where: { product: { id } } });
        break;
      case CATEGORY_MAP.laptop:
        detail = await this.laptopRepository.findOne({ where: { product: { id } } });
        break;
      case CATEGORY_MAP.pc:
        detail = await this.pcRepository.findOne({ where: { product: { id } } });
        break;
      case CATEGORY_MAP.ram:
        detail = await this.ramRepository.findOne({ where: { product: { id } } });
        break;
      case CATEGORY_MAP.gpu:
        detail = await this.gpuRepository.findOne({ where: { product: { id } } });
        break;
      case CATEGORY_MAP.psu:
        detail = await this.psuRepository.findOne({ where: { product: { id } } });
        break;
      case CATEGORY_MAP.drive:
        detail = await this.driveRepository.findOne({ where: { product: { id } } });
        break;
      case CATEGORY_MAP.motherboard:
        detail = await this.motherboardRepository.findOne({ where: { product: { id } } });
        break;
      case CATEGORY_MAP.cooler:
        detail = await this.coolerRepository.findOne({ where: { product: { id } } });
        break;
      case CATEGORY_MAP.case:
        detail = await this.caseRepository.findOne({ where: { product: { id } } });
        break;
      case CATEGORY_MAP.monitor:
        detail = await this.monitorRepository.findOne({ where: { product: { id } } });
        break;
      case CATEGORY_MAP.mouse:
        detail = await this.mouseRepository.findOne({ where: { product: { id } } });
        break;
      case CATEGORY_MAP["network-card"]:
        detail = await this.networkCardRepository.findOne({ where: { product: { id } } });
        break;
      case CATEGORY_MAP.headset:
        detail = await this.headsetRepository.findOne({ where: { product: { id } } });
        break;
      case CATEGORY_MAP.keyboard:
        detail = await this.keyboardRepository.findOne({ where: { product: { id } } });
        break;
      default:
        detail = null;
    }
  ...
  async searchProducts(keyword: string): Promise<Product[]> {
    ...
    return await this.productRepository.createQueryBuilder("product")
      .leftJoinAndSelect("product.category", "category")
      .where("product.isActive = :isActive", { isActive: true })
      .andWhere("LOWER(product.name) LIKE :keyword", { keyword: `%${keyword.toLowerCase()}%` })
      .orderBy("product.createdAt", "DESC")
      .getMany();
  }

  private async ensureRepositories() {
    if (!this.productRepository || !this.categoryRepository || !this.cpuRepository || !this.laptopRepository || !this.pcRepository || !this.ramRepository || !this.gpuRepository || !this.psuRepository || !this.driveRepository || !this.motherboardRepository || !this.coolerRepository || !this.caseRepository || !this.monitorRepository || !this.mouseRepository || !this.networkCardRepository || !this.headsetRepository || !this.keyboardRepository) {
